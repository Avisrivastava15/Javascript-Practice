...
function generate() {
        const itemValue = parseInt(item.value);  // Convert item.value to a number
    
        if (itemValue === 0) {
            alert("Value can't be zero");
        } else if (itemValue > paragraphs.length) {
            const randomIndex = Math.floor(Math.random() * paragraphs.length);
            dataContainer.innerHTML = `${paragraphs[randomIndex]}`;
        } else {
            const shuffleParagraphs = paragraphs.slice(); 
            shuffleArray(shuffleParagraphs); 
    
            const selectedParagraphs = shuffleParagraphs.slice(0, itemValue);
